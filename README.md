# Sample Application `deepstream-yolov9-triton-server-rtsp-out`  

The Purpose of this repository is to create a Deepstream/Triton-Server sample application that utilizes yolov9 models to perform inference on video files or RTSP streams. It then showcases the output on an RTSP URL, providing a straightforward demonstration of end-to-end AI processing.
...
### 3. Using Sample Application `deepstream-yolov9-triton-server-rtsp-out`  

To use this sample application, follow the steps below from within the container:

The sample application is installed on `/deepstream_python_apps/apps/deepstream-yolov9-triton-server-rtsp-out/`
...
```bash
python3 deepstream_yolov9-triton-server_rtsp_out.py -i <input_files> -m <model> -c <codec> -b <bitrate> [--rtsp-ts]
...
*   `-m`, `--model`: Choose 'yolov9'. (Default: 'yolov9')
